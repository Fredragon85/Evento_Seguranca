...
# Estética: Imagem de um segurança com fogo de artifício
...
def enviar_email(dest, nome, posto):
    try:
        msg = MIMEText(f"Olá {nome}, o seu turno foi confirmado: {posto}")
...
# --- CSS: BACKGROUND E BOTÃO INVISÍVEL ---
st.markdown(f"""
    <style>
    .stApp {{
...
# Botão dissimulado no canto inferior
...
if st.session_state.admin_mode:
    ...
    if senha == ADMIN_PASSWORD:
        t1, t2, t3 = st.tabs(["Gerar Turnos", "Inscrições", "Base de Dados"])
        
        with t1:
            texto = st.text_area("Cole o texto dos turnos:")
        ...
        with t3:
            ...
            if st.button("Limpar Tudo"):
                conn.execute("DELETE FROM escalas"); conn.execute("DELETE FROM configuracao_turnos")
                conn.commit(); st.rerun()
            conn.close()

# --- INTERFACE UTILIZADOR ---
st.title("🎆 Reserva de Turnos de Segurança")
    ...
    with st.form("registo"):
        ...
        if st.form_submit_button("Confirmar"):
            if not (nome and tel and mail):
                ...
                st.success("Sucesso! Verifique o email.")
                st.rerun()
