...
DB_NAME = 'sistema_v61_supreme.db'
        ...
        with closing(conn.cursor()) as c:
            ...
            c.execute('''CREATE TABLE IF NOT EXISTS escalas 
                         (id INTEGER PRIMARY KEY AUTOINCREMENT, posto TEXT, nome TEXT, telefone TEXT, email TEXT, 
                          status TEXT DEFAULT 'Pendente', pref_metodo TEXT, checkin INTEGER DEFAULT 0, pago INTEGER DEFAULT 0)''')
...
def multicanal_notify(email, tel, nome, posto, metodo):
    msg = f"Ola {nome}, o seu turno {posto} foi CONFIRMADO."
    if metodo in ["Email", "Ambos"]:
        try:
    ...
    if metodo in ["WhatsApp", "Ambos"]:
        try:
            Client(TWILIO_SID, TWILIO_TOKEN).messages.create(from_=TWILIO_WHATSAPP, body=msg, to=f"whatsapp:{tel}")
        except: pass
...
st.set_page_config(page_title="V61 SUPREME OMNI", layout="wide")

# --- SIDEBAR ---
with st.sidebar:
    st.title("🛡️ SUPREME v61")
    if st.session_state.user_email:
        st.success(f"Logado: {st.session_state.user_email}")
        if st.session_state.is_admin: st.warning("Acesso Administrador")
        if st.button("🔒 Sair"):
            st.session_state.user_email = None; st.session_state.is_admin = False; st.rerun()
    else:
        with st.form("login"):
            ...
            if st.form_submit_button("Entrar"):
                with get_db_conn() as conn:
                    ...
                    if res and check_password(s, res[0]):
                        st.session_state.user_email = e; st.session_state.is_admin = bool(res[1]); st.rerun()
                ...
                if p_id in reservados: c1.error(f"❌ {p_id} (Ocupado)")
                    ...
                    if st.session_state.user_email:
                        ...
                        if c2.button("Reservar", key=f"r_{p_id}"):
                            ...
                            st.success("Reserva enviada!"); time.sleep(0.5); st.rerun()

# --- MÓDULO: ÁREA DE CLIENTE ---
elif nav == "Área de Cliente":
    if not st.session_state.user_email: st.warning("Faça login para ver os seus turnos.")
    else:
        st.header("📋 O Meu Histórico")
        with get_db_conn() as conn:
            meus = conn.execute('''SELECT e.posto, e.status, e.checkin, e.pago, t.lat, t.lon 
                                   FROM escalas e LEFT JOIN configuracao_turnos t ON e.posto = t.posto 
                                   WHERE e.email = ?''', (st.session_state.user_email,)).fetchall()
            for p, stt, chk, pago, lat, lon in meus:
                with st.container(border=True):
                    c1, c2 = st.columns([3, 1])
                    c1.write(f"**Posto:** {p}")
                    c1.caption(f"Status: {stt} | Pago: {'✅' if pago else '⏳ Pendente'}")
...
if st.session_state.is_admin:
    with st.expander("🛠️ PAINEL ADMINISTRATIVO", expanded=True):
                ...
                if txt:
                    ...
                    for l in txt.split('\n'):
                        ...
                        if "DIA" in l.upper(): dia = l
                            ...
                            p_list.append({"ID": f"{dia} | {loc} | {l}", "Loc": loc})
                ...
                if st.button("✅ Publicar"):
                    with get_db_conn() as conn:
                        for i in st.session_state.preview_data:
                            conn.execute("INSERT OR IGNORE INTO configuracao_turnos VALUES (?,?,?,?,?)", (i['ID'], "PSG", i['Loc'], 39.2081, -8.6277))
        ...
        with t3:
            with get_db_conn() as conn:
                concluidos = conn.execute("SELECT id, nome, posto, pago FROM escalas WHERE status='Confirmado' AND checkin=1").fetchall()
                for id_e, nome, posto, pago in concluidos:
                    with st.container(border=True):
                        c1, c2 = st.columns([4, 1])
                        c1.write(f"**{nome}** concluiu: {posto}")
                        if not pago:
                            if c2.button("Pagar", key=f"pay_{id_e}"):
                                conn.execute("UPDATE escalas SET pago=1 WHERE id=?", (id_e,)); st.rerun()
            ...
            with get_db_conn() as conn:
                ...
                for u_e, u_n, adm in users:
                    with st.expander(f"{u_n} ({u_e})"):
                        ...
                        if not adm and st.button("Tornar Admin", key=f"p_{u_e}"):
    ...
    with st.form("r"):
        ...
        if st.form_submit_button("Criar Conta"):
            with get_db_conn() as conn:
                conn.execute("INSERT OR REPLACE INTO clientes (email, senha, nome, telefone) VALUES (?,?,?,?)", (e, hash_password(s).decode('utf-8'), n, t))
